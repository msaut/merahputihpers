...
<div class="container-fluid px-4">
    <h1 class="mt-4">Tambah Banner / Iklan</h1>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-lg-8">
                        ...
                        <div class="row">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="posisi" id="banner-position" class="form-select @error('posisi') is-invalid @enderror" required>
                                        @foreach (config('banner.positions') as $key => $config)
                                            <option value="{{ $key }}" {{ old('posisi') === $key ? 'selected' : '' }}>{{ $config['label'] }}</option>
                                        @endforeach
                                    </select>
                                    @error('posisi')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" min="0" name="urutan" class="form-control @error('urutan') is-invalid @enderror" value="{{ old('urutan', 0) }}">
                                    @error('urutan')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex gap-3 mt-2">
                                        <label class="form-check-label">
                                            <input type="radio" name="target" value="_self" {{ old('target', '_blank') === '_self' ? 'checked' : '' }}> Tab yang sama
                                        </label>
                                        <label class="form-check-label">
                                            <input type="radio" name="target" value="_blank" {{ old('target', '_blank') === '_blank' ? 'checked' : '' }}> Tab baru
                                        </label>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="alt_text" class="form-control @error('alt_text') is-invalid @enderror" value="{{ old('alt_text', old('judul')) }}">
                            @error('alt_text')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card border-light bg-light h-100">
                            <div class="card-body">
                                <h6 class="fw-bold">Upload Gambar Banner</h6>
                                <input type="file" id="banner-image" name="gambar" accept="image/jpeg,image/png,image/webp" class="form-control @error('gambar') is-invalid @enderror" required>
                                @error('gambar')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                                ...
                                <div id="banner-preview" class="mt-3 text-center border rounded p-2 bg-white" style="min-height:120px; display:flex; align-items:center; justify-content:center;">
                                    <span class="text-muted small">Preview banner akan muncul di sini.</span>
                                </div>

                                <div class="mt-4 small text-muted">
                                    <div class="fw-bold mb-2">Ukuran yang disarankan:</div>
                                    <ul class="mb-0 ps-3">
                                        <li>Header: 970 × 250 px</li>
                                        <li>Leaderboard: 728 × 90 px</li>
                                        <li>Sidebar: 300 × 250 px</li>
                                        <li>Mobile: 320 × 100 px</li>
                                        <li>Square: 300 × 300 px</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-danger">Simpan Banner</button>
...
<script>
    const positionConfig = {
        header: { width: 970, height: 250, label: 'Header' },
        ...
        square: { width: 300, height: 300, label: 'Square' }
    };
    ...
    function showPreview(file) {
        if (!file) {
            previewBox.innerHTML = '<span class="text-muted small">Preview banner akan muncul di sini.</span>';
            return;
        }
    ...
    function checkImageDimensions(file) {
        ...
        const config = positionConfig[pos] || { width: 0, height: 0, label: pos };
